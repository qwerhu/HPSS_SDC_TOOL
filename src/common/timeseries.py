...
class TimeSeries:
    def __init__(self, db, bucket, interval, alignment=True):
        ...
        # utils.indexing(self._db, [('id', 2), ('timetag', 1)])
    ...
    def upserts(self, rcds, w=1):
        ...
        updates = []
        updates_init = []
        for m, _id, v, t in rcds:
            padding, index, metric = utils.pim(t, self.padding, self.interval)
            c_key = '{}_{}'.format(_id, padding)
            o = None
            # if c_key in self.cache_dict:
            #     o = self.cache_dict.get(c_key)
            # else:
            ...
            if o is not None:
                ...
                if len(self.cache_dict) > 10000:
                    self.cache_dict = {}
                self.cache_dict[c_key] = o
            tpl = {'time': 0, 'motime': 0, 'value': 0}
            node = {'time': metric, 'motime': t, 'value': v}
            if not o:
                o = UpdateOne({'id': _id, 'timetag': {'$eq': padding}}, {'$setOnInsert': {'id': _id,
                                    'timetag': padding,
                                    'starttime': t,
                                    'endtime': t,
                                    'values': [ tpl for _ in range(self.limits) ]}}, upsert=True)
                updates_init.append(o)
            # else:
            ...
            op = UpdateOne(query, {'$max': {'endtime': metric}, '$min': {'starttime': metric}, '$set': {'values.%d' % index: node}})
            updates.append(op)
        if len(updates_init) > 0:
            self.__db().bulk_write(updates_init, ordered=False, bypass_document_validation=True)
        if len(updates) > 0:
            self.__db().bulk_write(updates, ordered=False, bypass_document_validation=True)
